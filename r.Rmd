...
n = 100
p = as.numeric(data[
  data$state == "New York" & data$county == "Westchester" & data$date == max(data$date),
  "cases.active.perM"
])
```
...
\begin{align}
1 - P \left( q > x | n, prob \right) &= 1 - {n \choose x} \cdot prob^x \cdot \left( 1 - prob \right)^{\left(n - x\right)} \\
1 - P \left( q > `r x` | n = `r n`, prob = `r round(p, 0)` \cdot 10^{-6} \right) &= 1 - \frac{`r n`!}{`r x`! \left( `r n` - `r x` \right) !} \cdot \left( `r round(p, 0)` \cdot 10^{-6} \right) ^ `r x` \cdot \left ( 1 - `r round(p, 0)` \cdot 10^{-6} \right)^{\left(`r n` - `r x`\right)}
\end{align}

```{r}
cdf <- data.frame()
for (i in c(1, 5, 10, 25, 100, 250, 500, 1000, 2500, 5000, 10000)) {
  
  temp <- data.frame(
    n = i, 
    prob = pbinom(q = x, size = i, prob = p * 10^-6, lower.tail = FALSE) * 100
  )
...
```{r, echo = FALSE}
kable(
  caption = "Probability of Exposure to AT LEAST n Actively Contageous Persons. Assumes 9-Day Infectious Period",
  x = cdf %>% mutate(prob = round(prob, 1))
)
```
