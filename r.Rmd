...
data.state <- data %>%
  ...
  mutate(
    cases.active = rollsum(cases.new, k = 14, fill = 0, align = "right"),
    cases.active.perCap = rollsum(cases.new.perCap, k = 14, fill = 0, align = "right"),
    cases.active.perM = rollsum(cases.new.perM, k = 14, fill = 0, align = "right")
  )
```
...
wedding <- merge(
  ...
  x = data.frame(
    state = c("Wisconsin", "South Dakota", "Indiana", "Arizona", "Nebraska"),
    guests = c(4, 8, 3, 4, 200)
  ),
...
cases.active.perM.wavg <- weighted.mean(
