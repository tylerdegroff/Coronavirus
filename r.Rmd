...
1 - P(q > x = 0 | n = `r sum(wedding$guests)`, p = `r round(weighted.mean(x = wedding$cases.active.perM, w = wedding$guests), 0)` \cdot 10^{-7}) &= 1 - \left[ \frac{`r sum(wedding$guests)`!}{0! \cdot (`r sum(wedding$guests)` - 0)!} \right] \cdot {\left[`r round(weighted.mean(x = wedding$cases.active.perM, w = wedding$guests), 0)` \cdot 10^{-7} \right]}^0 \cdot \left[1 - `r round(weighted.mean(x = wedding$cases.active.perM, w = wedding$guests), 0)` \cdot 10^{-7} \right]^{`r sum(wedding$guests)` - 0}
\end{align}
...
\begin{align}
1 - P(q > x | n, p) &= 1 - {n \choose x} p^x \left( 1 - p \right)^{\left(n - x\right)} \\
1 - P(q > x | n = `r eventSize`, p = `r round(caseDensity, 0)` \cdot 10^{-7}) &= 1 - \left[ \frac{`r eventSize`!}{x! \cdot (`r eventSize` - x)!} \right] \cdot {\left[`r round(caseDensity, 0)` \cdot 10^{-7} \right]}^x \cdot \left[1 - `r round(caseDensity, 0)` \cdot 10^{-7} \right]^{`r eventSize` - x}
\end{align}
